...
class Heroes{
  ...
  set selectedTabId(String id){
    _selectedTabId = id;
    _content.querySelectorAll(".selectedTabButton").classes.remove("selectedTabButton");
  ...
  void paint(){
    ...
    addSources();
    reduceSources();
    _content.querySelectorAll(".heroWidgetTabs ul li").onClick.capture((Event e){
      Element target = e.target;
      selectedTabId = target.id;
    });
    if(NumberInputElement.supported){
      _content.querySelector("._heroExperience").replaceWith(
          new NumberInputElement()
          ..value = _selected.data.experience.toString()
          ..classes.add("_heroExperience")
            ..min = "1"
            ..onChange.capture((e){experienceChanged();})
      );
    }else{
      ...
      exp.onChange.capture((e){experienceChanged();});
      Element plus = new Element.tag("button")..innerHtml="+"
          ..onClick.capture((e){
            exp.value = (int.parse(exp.value)+1).toString();
            experienceChanged();
          });
      Element minus = new Element.tag("button")..innerHtml="-"
          ..onClick.capture((e){
            exp.value = (int.parse(exp.value)-1).toString();
            experienceChanged();
          });
      
      _content.querySelector(".heroLevel")
      ..append(plus) 
      ..append(minus);
  ...
  void experienceChanged(){
    ...
    _selected.recalc();
    _content.querySelector("._heroLevel").innerHtml = _selected.calculated.level.toString();
  }
  ...
  renderTabs() {
    ...
    for(Hero hero in _heroes){
        ...
        ..classes.add("herotab")
        ..text = hero.data.name
        ..onMouseDown.capture((e){selectHero(hero);})
        ..onMouseMove.capture((e){e.preventDefault();return false;});
        hero.tab = heroTab;
    ...
    _tabs.append(new Element.div()
          ...
          ..onMouseUp.capture((event){event.preventDefault();return false;})
        );
  }
