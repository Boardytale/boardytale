...
void packSDK(String directory, {bool removeComments: true}){
  List<String> parts = (new File("template/complete.ts")).readAsStringSync().split("//@marker1");
  String packed = parts[1];
  List<String> lines = parts[2].split(";");
  for(String line in lines){
    if(!line.contains("'"))continue;
    String destinationFileString = (new File("template/"+line.split("'")[1]+".ts")).readAsStringSync();
    packed+="\r";
    packed+="export";
  ...
  if(removeComments){
    ...
    for(int i=1;i<commentSeparated.length;i++){
      ...
      packed+=commentSeparated[i].replaceFirst(comment, "").replaceFirst("\r", "");
    }
  }
  if(directory==null) {

    packed = packed.replaceAll("} from './storage_identifiers'", "} from './template/storage_identifiers'");
